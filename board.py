...
class Board:
    ...
    def reset(self):
        ...
        self.w_pieces = {
            "rook1": Rook((0, 7), "w", 1),
            "rook2": Rook((7, 7), "w", 2),
            "knight1": Knight((1, 7), "w", 1),
            "knight2": Knight((6, 7), "w", 2),
            "bishop1": Bishop((2, 7), "w", 1),
            "bishop2": Bishop((5, 7), "w", 2),
            "queen1": Queen((3, 7), "w", 1),
            "king1": King((4, 7), "w", 1),
            "pawn1": Pawn((0, 6), "w", 1),
            "pawn2": Pawn((1, 6), "w", 2),
            "pawn3": Pawn((2, 6), "w", 3),
        ...
        self.b_pieces = {
            "rook1": Rook((0, 0), "b", 1),
            "rook2": Rook((7, 0), "b", 2),
            "knight1": Knight((1, 0), "b", 1),
            "knight2": Knight((6, 0), "b", 2),
            "bishop1": Bishop((2, 0), "b", 1),
            "bishop2": Bishop((5, 0), "b", 2),
            "queen1": Queen((3, 0), "b", 1),
            "king1": King((4, 0), "b", 1),
            "pawn1": Pawn((0, 1), "b", 1),
            "pawn2": Pawn((1, 1), "b", 2),
            "pawn3": Pawn((2, 1), "b", 3),
    ...
    def draw_piece(self, color, piece, pos):
        self.screen.blit(
            pygame.image.load(piece.get_piece_img(color, piece)),
            (
                pos[0] * SQUARE_SIZE + PIECE_PADDING,
                pos[1] * SQUARE_SIZE + PIECE_PADDING,
            ),
        )
    ...
    def draw_borders(self):
        for i in range(WIDTH // SQUARE_SIZE + 1):
            pygame.draw.line(
                self.screen, BORDER_COLOR, (i * SQUARE_SIZE, 0), (i * SQUARE_SIZE, 640)
            )
            pygame.draw.line(
                self.screen, BORDER_COLOR, (0, i * SQUARE_SIZE), (640, i * SQUARE_SIZE)
            )
    ...
    def check_moves(self, moves):
        ...
        for i in range(len(moves)):
            ...
            _piece_board, _piece = self._do_move(_piece, moves[i], _piece_board, _board)
    ...
    def _do_move(self, piece, move, piece_board, board):
        ...
        """
        piece_board[piece.pos[0]][piece.pos[1]] = board[piece.pos[0]][piece.pos[1]]
    ...
    def checkmate(self, color):
        if self.check_moves(
            self.get_all_possible_moves(color, self.piece_board, self.pieces)
        ) == [] and self.is_in_check(color, self.piece_board, self.pieces):
            return True
        return False
    ...
    def stalemate(self, color):
        if self.check_moves(
            self.get_all_possible_moves(color, self.piece_board, self.pieces)
        ) == [] and not self.is_in_check(color, self.piece_board, self.pieces):
            return True
        return False
    ...
    def handle_click(self):
        ...
        self.selected_piece = (
            ...
            if self.selected_square in self.b_pieces.values()
            or self.selected_square in self.w_pieces.values()
            else self.selected_piece
        )
        # deselect piece
        if (
            self.selected_piece == self.selected_piece_prev
